...
export async function handleDirectUpload(req, res) {
  try {
    ...
    const userDescription =
      req.body.description || req.body.userNote || req.body.user_note || "";
    let locationData = {
      address: "",
      latitude: null,
      longitude: null,
    ...
    if (
      ...
      if (!Number.isFinite(locationData.longitude))
        locationData.longitude = null;
    }
    ...
    const newComplaint = new Complaint({
      ...
      status: "AI Verified",
      reportCount: 1,
      duplicateOf: duplicateCheck.isDuplicate
        ? duplicateCheck.possibleDuplicate.complaintId
        : null,
      history: [
        {
          status: "Submitted",
    ...
    return res.status(200).json({
      ...
      location: aiResult.location,
      imageUrl,
      possibleDuplicate: duplicateCheck.isDuplicate
...
/**
 * Handle rephrasing user description with AI
 * Route: POST /upload/rephrase, POST /ai/rephrase
 */
export async function handleRephraseDescription(req, res) {
  try {
    const { text, location } = req.body || {};
    if (!text || !text.trim()) {
      return res
        .status(400)
        .json({ error: "Description text is required to rephrase" });
    }

    const result = await rephraseComplaintDescription({ text, location });
    return res.status(200).json({
      success: true,
      original: text,
    ...
    console.error("[UploadController] handleRephraseDescription error:", error);
    return res.status(500).json({ error: "Failed to rephrase description" });
  }
}
